    ...
    const fetchData = async () => {
        ...
        try {
            ...
            for (let i = 0; i < tournaments.length; i++) {
                ...
                const usersTournaments = await fetch(`ws/tournaments/${tournament.id}/users`);
                const test = await usersTournaments.json();

                console.log(test);

                // Identifiants des tournois auxquels l'utilisateur a déjà participé
                const joinedTournamentsIds = [1,2];
                ...
                tournamentDiv.innerHTML = `
                    ...
                    <div class="allBtn">
                        ...
                        `<button class="btn invalid" onClick="leaveTournament(${tournament.id})">Quitter</button><button class="btn sec" onClick="showTournament(${tournament.id})">Voir</button>`:
